...
import { createBrand } from '@/api/othersApi'
...
const AddingBrand = () => {
  ...
  const sendData = async (e) => {
    e.preventDefault()
    await createBrand({ title: value }, token)
    setValue('')
  }
  ...
  return (
    <form className="grid items-center justify-center h-[50vh]">
      <div>
        <label htmlFor="name">
          <h2 className="text-xl font-bold">Add A New Brand</h2>
        </label>
        <input
          type="text"
          id="name"
        ...
        >
          This will be added to the Brand List.
        </p>
        <button
          type="submit"
